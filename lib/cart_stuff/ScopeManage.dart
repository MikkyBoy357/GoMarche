    ...
    try {
      await this._db.transaction((tx) async {
        for (var i = 0; i < data.length; i++) {
          ...
          Data d = new Data();
          d.id = i + 1;
          d.name = data[i]["name"].toString();
          d.image = data[i]["image"].toString();
          d.price = double.parse(data[i]["price"].toString());
          d.fav = data[i]["fav"] == "true" ? true : false;
          d.rating = double.parse(data[i]["rating"].toString());
          try {
            var qry =
